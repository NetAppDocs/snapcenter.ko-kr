...
* Linux 또는 AIX 호스트에 Java 17(SnapCenter 6.2.1 이상 버전) 또는 Java 11(SnapCenter 6.2 이하 버전)이 설치되어 있어야 합니다.
+
** Linux에서는 Oracle, OpenJDK 또는 SAP JDK의 Java가 지원됩니다.
** AIX용 IBM Java입니다. 다운로드는  https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeru RunTimes 다운로드"^]에서 할 수 있습니다.
+

NOTE: Linux 호스트에 인증된 Java 17(SnapCenter 6.2.1 이상 버전용) 또는 Java 11(SnapCenter 6.2 이하 버전용)만 설치되어 있는지 확인하십시오.
...
필요한 소프트웨어 패키지
 a| 
* SnapCenter 6.2.1 이상용 Java 17(Oracle Java, OpenJDK 또는 SAP JDK) 또는 SnapCenter 6.2 이하용 Java 11



NOTE: Linux 호스트에 SnapCenter 6.2.1 이상 버전에는 인증된 Java 17만, SnapCenter 6.2 이하 버전에는 Java 11만 설치되어 있는지 확인하십시오.
...
*  `/etc/ssh/sshd_config` 파일을 편집하여 메시지 인증 코드 알고리즘을 구성합니다. MACs hmac-sha2-256 및 MACs hmac-sha2-512.
+
...
필요한 소프트웨어 패키지
 a| 
* IBM Java 17 for SnapCenter 6.2.1 이상 또는 IBM Java 11 for SnapCenter 6.2 이하
...
CAUTION: 호스트를 배포하고 플러그인을 설치한 후에도 자격 증명을 생성할 수 있지만, *모범 사례*는 SVM을 추가한 후 호스트를 배포하고 플러그인을 설치하기 전에 자격 증명을 생성하는 것입니다.
...
NOTE: 자격 증명을 생성하는 동안 잘못된 정보를 입력하면 오류 메시지가 표시됩니다. *취소*를 선택한 다음 다시 시도하십시오.

.단계
. 왼쪽 탐색 창에서 * 리소스 * 를 선택한 다음 목록에서 적절한 플러그인을 선택합니다.
. 리소스 페이지의 * 보기 * 목록에서 * 데이터베이스 * 를 선택합니다.
. image:../media/filter_icon.gif["필터 아이콘"]을 선택한 다음 호스트 이름과 데이터베이스 유형을 선택하여 리소스를 필터링합니다.
+
그런 다음 image:../media/filter_icon.gif["필터 아이콘"]를 선택하여 필터 창을 닫을 수 있습니다.

. 데이터베이스를 선택한 다음 *Database Settings* > *Configure Database*를 선택합니다.
...
+
자세한 내용은 link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["RAC 설정의 1차 노드"^]을(를) 참조하십시오.
