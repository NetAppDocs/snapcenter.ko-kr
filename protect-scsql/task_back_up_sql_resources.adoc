...
.이 작업에 대해
* 백업 작업을 최적화하려면 DNS 서버에 Windows 클러스터 이름 및 IP 주소의 역방향 조회 레코드를 만들어야 합니다.
...
+
* * 를 선택하여 정책 마법사를 시작하여 정책을 생성할 수image:../media/add_policy_from_resourcegroup.gif["리소스 그룹 양식 정책 추가"]있습니다.
...
.. 확인 일정을 구성할 정책을 선택한 다음 * * * 를image:../media/add_policy_from_resourcegroup.gif["리소스 그룹 양식 정책 추가"]선택합니다.
