...
* 독립 실행형 및 데이터베이스 가용성 그룹 구성에는 Microsoft Exchange Server를 사용해야 합니다. 지원되는 Microsoft Exchange Server 버전은 link:https://imt.netapp.com/imt/#search["상호 운용성 매트릭스 도구(IMT)"^]을 참조하십시오.
...
* 방화벽에서 다음 포트가 차단되지 않았는지 확인하십시오. 그렇지 않으면 호스트 추가 작업이 실패합니다. 이 문제를 해결하려면 동적 포트 범위를 구성해야 합니다. 자세한 내용은 https://docs.microsoft.com/en-us/troubleshoot/windows-server/networking/configure-rpc-dynamic-port-allocation-with-firewalls["Microsoft 설명서"^]를 참조하십시오. 지원되는 Microsoft Exchange Server 버전은 link:https://imt.netapp.com/imt/#search["상호 운용성 매트릭스 도구(IMT)"^]를 참조하십시오.
+
** Windows 2019의 경우 포트 범위 49152-65536
** Windows 2016 및 Exchange 2016의 경우 포트 범위 50000 - 51000
** Windows 2012 R2 및 Exchange 2013의 포트 범위 6000 - 6500
...
필요한 소프트웨어 패키지
 a| 
* ASP.NET Core Runtime 10.0.7 Hosting Bundle(또는 이후 버전)
* PowerShell 7.6.1 이상
* Java 17(Oracle Java, OpenJDK 또는 SAP JDK) 또는 Java 11(SnapCenter 6.2 이하 버전용)
