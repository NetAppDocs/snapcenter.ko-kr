...
워크플로 이미지에는 F5 로드 밸런서를 사용하여 고가용성을 위해 SnapCenter 서버를 구성하는 단계가 나와 있습니다. 자세한 지침은 을 참조하십시오 https://["F5 로드 밸런서를 사용하여 고가용성을 위해 SnapCenter 서버를 구성하는 방법"^].
...
* 제거 - SmServerCluster
+
자세한 내용은 을 참조하십시오 https://["SnapCenter 소프트웨어 cmdlet 참조 가이드"^].
