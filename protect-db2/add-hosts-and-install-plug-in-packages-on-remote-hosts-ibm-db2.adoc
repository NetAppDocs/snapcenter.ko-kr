...
. 제출 * 을 클릭합니다.
+
Skip prechecks 확인란을 선택하지 않은 경우 호스트는 호스트가 플러그인 설치 요구 사항을 충족하는지 확인합니다. 디스크 공간, RAM, PowerShell 버전, NET 버전, 위치(Windows 플러그인의 경우) 및 Java 11(Windows 및 Linux 플러그인의 경우)은 최소 요구 사항에 따라 검증됩니다. 최소 요구 사항이 충족되지 않으면 적절한 오류 또는 경고 메시지가 표시됩니다.
