...
summary: MySQL 백업 복원에는 SnapCenter 서버와의 연결 세션 시작, 백업 목록 및 백업 정보 검색, 백업 복원 등이 포함됩니다. 
---
= PowerShell cmdlet을 사용하여 MySQL 데이터베이스를 복원합니다
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
MySQL 백업 복원에는 SnapCenter 서버와의 연결 세션 시작, 백업 목록 및 백업 정보 검색, 백업 복원 등이 포함됩니다.
...
NOTE: AppObjectId는 "호스트\플러그인\UID"입니다. 여기서 UID=<instance_name>는 수동으로 검색된 MySQL 인스턴스 리소스용이고 UID=<instance_name>\<databse_name>는 MySQL 데이터베이스 리소스용입니다. Get-SmResources cmdlet에서 ResourceID를 가져올 수 있습니다.
